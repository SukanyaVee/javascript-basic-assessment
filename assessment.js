...
// var firstHero = heroes.slice(0,1)
...
{
	var newarr = arr.filter(elem=>elem!=str)
	return newarr
}
...
{
	if (parseFloat(para,10))
	{
	return parseFloat(para)+parseFloat(para);
	}
	else
	{
		return para+para
	}
	// (parseFloat(para,10)) ? parseFloat(para)+parseFloat(para) : para+para
}
